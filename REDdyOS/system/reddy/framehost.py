...
def setup(name, commons):
    ...
    apps[name] = {
                 ...
                 'horizontal_pager': ezd.horizontal.pager((position[0] + size[0] - 11, position[1] + size[1] / 2 + 20, 10, size[1]),
...
def exit(ID):
    global rem
    try:
        apps[ID][3] = pe.display.display_reference
    except:
        return
    try:
    ...
    pe.display.blit(apps[ID][3], (pos[0], pos[1] + 20))
    ...
    if True:
        ...
        if drag or lastM:
            pe.draw.rect(pe.colors.red,
                         (app[1].window_pos[0] + app[1].window_size[0] - 35, app[1].window_pos[1], 35, 20), 0)
        else:
            pe.button.rect((app[1].window_pos[0] + app[1].window_size[0] - 35, app[1].window_pos[1], 35, 20),
        ...
        try:
            ...
            if rem != None:
                pe.display.context(rem)
            return
        apps[ID][3] = pe.Surface(app[1].window_size)
        buldge = 3
        pe.draw.rect(pe.colors.black,
                     (app[1].window_pos[0], app[1].window_pos[1], app[1].window_size[0], app[1].window_size[1] + 20),
                     buldge)
        pe.draw.line(pe.colors.black, (app[1].window_pos[0], app[1].window_pos[1] + 20),
                     (app[1].window_pos[0] + app[1].window_size[0], app[1].window_pos[1] + 20), buldge)
...
def draw(ID):
    ...
    apps[ID][1].window_pos[0], apps[ID][1].window_pos[1], apps[ID][1].window_size[0] - 35, 20)).display()
