...
def fit(text, color, rect, g=25):
    return pe.text.Text(text, 'freesansbold.ttf', int((rect[0] - rect[2]) / g), pe.math.center(rect), (color, None))
...
def cram(text, color, rect):
    ...
    if maxX < len(text):
        ...
        text = text[0:maxX - 4] + "..."
    if cram:
        textV = pe.text.Text(text, 'freesansbold.ttf', maxY, (rect[0], rect[1]), (color, None))
        return align.topleft(textV)
    else:
        return pe.text.Text(text, 'freesansbold.ttf', maxY, pe.math.center(rect), (color, None))
